# E04
#  4つのネームスペース(仮想PC) ns1,ns2,ns3,swhubを作成しブリッジ(仮想ネット
#  ワークスイッチ)br1を作成する。それぞれに仮想イーサネットインタフェースを設定し
...
# 5:仮想ネットワークインタフェースにIPアドレスを設定した状態
# 6:仮想ネットワークインタフェースにMACアドレスを設定した状態
# 7:ネットワークネームスペースの仮想ネットワークインタフェースを有効(UP)にした状態
# 8:ブリッジとブリッジの仮想ネットワークインタフェースを有効(UP)にした状態
# 9:仮想ネットワークインタフェースをブリッジに配置した状態:w
...
function fn_exp1() {
cat << END
# ネットワークネームスペースを4つ作成します。ns1,ns2,ns3,swhubはホストOSの
# Linuxからはネットワーク的に独立しています。ここではns1,ns2,ns3を仮想PCとして,
# swhubをスイッチグハブとして扱います。ハブに３つのPCが接速された状態です。
# 
# sudo ip netns add ns1
# sudo ip netns add ns2
# sudo ip netns add ns3
# sudo ip netns add swhub
#
...
function fn_exp2() {
cat << END
# swhubの中にブリッジ(br1)を作成します。ブリッジは仮想ネットワークスイッチであり、
# 実際のネットワークデバイスと仮想ネットワークデバイスを接続することができます。
# つまり仮想PC同士を接続するスイッチングハブの機能を実現しています。
#
# sudo ip netns exec swhub ip link add dev br1 type bridge
# sudo ip netns exec swhub ip link set br1 up
#
...
function fn_exp4() {
cat << END
# 仮想ネットワークインタフェースをネットワークネームスペース(ns1,ns2,ns3)に配置する。
...
# sudo ip link set br1-veth0 netns swhub
# sudo ip link set br1-veth1 netns swhub
# sudo ip link set br1-veth2 netns swhub
#
...
function fn_exp5() {
cat << END
# 仮想PCの仮想ネットワークインタフェースにIPアドレスを設定する。
# インタフェースは無効(DOWN)な状態である。よってまだ通信はできない。
# ブリッジ側の仮想ネットワークインタフェースにはIPアドレスを設定
# する必要はない。
#
...
# 「ip netns exec」コマンドはネットワークネームスペース内でコマンドを実行する
# ためのコマンドです。ns1,ns2,ns3はネットワーク的に独立しているために、ns1内に
# あるns1-veth0にMACアドレスを設定するためには、ns1の内部でip linkコマンド
# を実行する必要があります。
# 「ip link set dev dev ネットワークインタフェース MACアドレスを」は、
# ネットワークインタフェースにMACアドレスを設定します。 
# まだ仮想ネットワークインタフェースは無効(DOWN)な状態です。
# 
END
}
...
function fn_exp8() {
cat << END
# ブリッジの仮想ネットワークインタフェースを有効化(UP)します。
#
# sudo ip netns exec swhub ip link set br1-veth0 up
# sudo ip netns exec swhub ip link set br1-veth1 up
# sudo ip netns exec swhub ip link set br1-veth2 up
#
# 「ip link set <device> up」コマンドはネットワークインタフェースを有効化 
# (UP)します。
#
...
function fn_exp9() {
cat << END
# 仮想ネットワークインタフェースをブリッジに接続します。
#
# sudo ip netns exec swhub ip link set br1-veth0 master br1
# sudo ip netns exec swhub ip link set br1-veth1 master br1
# sudo ip netns exec swhub ip link set br1-veth2 master br1
#
...
仮想ネットワークインタフェースにMACアドレスを設定
ネットワークネームスペースの仮想ネットワークインタフェースを有効化
ブリッジの仮想ネットワークインタフェースを有効化
...
do
    ...
    1) #ネットワークネームスペースを作成する
        ...
        fn_exp1
        ;;
    2) #ブリッジを作成して、有効(UP)状態にする
        echo sudo ip netns exec swhub ip link add dev br1 type bridge
        echo sudo ip netns exec swhub ip link set br1 up
        echo ''
        sudo ip netns exec swhub ip link add dev br1 type bridge
        sudo ip netns exec swhub ip link set br1 up
        stat=2
        ...
        fn_exp2
        ;;
    3) #仮想ネットワークインタフェースと仮想ネットワークケーブルを作成する
    ...
    4) #仮想ネットワークインタフェースをネットワークネームスペースに配置する
        ...
        echo sudo ip link set br1-veth0 netns swhub
        echo sudo ip link set br1-veth1 netns swhub
        echo sudo ip link set br1-veth2 netns swhub
        echo ''
        sudo ip link set ns1-veth0 netns ns1
        sudo ip link set ns2-veth0 netns ns2
        sudo ip link set ns3-veth0 netns ns3
    ...
    5) #仮想ネットワークインタフェースにIPアドレスを設定する
        ...
        fn_exp5
        ;;
    6) #仮想ネットワークインタフェースにMACアドレスを設定する
        echo sudo ip netns exec ns1 ip link set dev ns1-veth0 address 00:00:5E:00:53:01
        echo sudo ip netns exec ns2 ip link set dev ns2-veth0 address 00:00:5E:00:53:02
        echo sudo ip netns exec ns3 ip link set dev ns3-veth0 address 00:00:5E:00:53:03
        echo ''
        sudo ip netns exec ns1 ip link set dev ns1-veth0 address 00:00:5E:00:53:01
        sudo ip netns exec ns2 ip link set dev ns2-veth0 address 00:00:5E:00:53:02
        sudo ip netns exec ns3 ip link set dev ns3-veth0 address 00:00:5E:00:53:03
    ...
    7) #ネットワークネームスペースの仮想ネットワークインタフェースを有効にする
        ...
        fn_exp7
        ;;
    8) #ブリッジの仮想ネットワークインタフェースを有効にする
        echo sudo ip netns exec swhub ip link set br1-veth0 up
        echo sudo ip netns exec swhub ip link set br1-veth1 up
        echo sudo ip netns exec swhub ip link set br1-veth2 up
        echo ''
        sudo ip netns exec swhub ip link set br1-veth0 up
        sudo ip netns exec swhub ip link set br1-veth1 up
        sudo ip netns exec swhub ip link set br1-veth2 up
    ...
    10) #ネットワークネームスペースを確認する
        ...
        ;;
    11) #ブリッジを確認する
        echo sudo ip netns exec swhub ip link list br1
        echo ''
        sudo ip netns exec swhub ip link list br1
        ;;
    ...
    13) #ブリッジに接続された仮想ネットワークインタフェースを確認する
        echo sudo ip netns exec swhub ip link list master br1
        echo ''
        sudo ip netns exec swhub ip link list master br1
        ;;
    14) #pingを実行(n1->n2)する
        echo "-----------------------------------------------------------"
